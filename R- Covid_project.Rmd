...
#Part A: Exploratory Data Analysis and Predictive Data Analysis Using R
...
```
At the end of this table we can find a row with high total cases and total deaths, this is our outlier row. Since we have no way of finding out the correct case data from this date, we will omit the row.

```{r}
#Delete outlier data row
negative_val_loop <- negative_val_loop[-c(477), ]
```
...
```{r}
#Plot a line graph using the clean covid data examining new cases over time
ggplot(data = negative_val_loop, aes(x = date, y = new_deaths, group = location, color = location))+
  geom_line(linewidth = 0.5)+
  #Split plot by country, give each plot its own scale
  facet_grid(rows = vars(location), scales = "free")
```
We can see Iran has another row with outlier data. Lets examine it closer.

```{r}
#filter Iran data by rows with 0 new_deaths.
tail(filter(negative_val_loop, new_deaths == 0 & location == "Iran"))
```
At the end of this table we can find a row with high total cases and total deaths, this is our outlier row. Since we have no way of finding out the correct case data from this date, we will omit the row.

```{r}
#Delete outlier row
negative_val_loop <- negative_val_loop[-c(555), ]
```
...
covid_data_clean <- negative_val_loop
covid_data_clean
```
...
covid_data_clean <- covid_data_clean %>%
  mutate(
    GDP_status = ifelse(
      mean_gdp, #mean of GDP_per_capita
      "Higher", 
      "Lower"
    )
  )
...
ggplot(data = covid_data_clean, aes(x = date, y = daily_infected_case_rate, group = GDP_status, color = GDP_status))+
  geom_line()+
  #Scale x-axis by month
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%y")+
  #Add title and legend description
  labs(title = "Daily Infected Case Rate by GDP Status", colour = "GDP Status")+
  #Add y-axis label
  ylab("Daily Infected Case Rate")+
  #Add x-axis label
  xlab("Date")
...
ggplot(data = covid_data_clean, aes(x = date, y = daily_death_rate, group = GDP_status, color = GDP_status))+
  geom_line()+
  #Scale x-axis by month
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%y")+
  #Add title and legend description
  labs(title = "Daily Death Rate by GDP Status", colour = "GDP Status")+
  #Add y-axis label
  ylab("Daily death Rate")+
  #Add x-axis label
  xlab("Date")
...
#Plotting the model and the data together
ggplot(covid_aus_filter, aes(date, new_cases)) +
  geom_point() +
  geom_line(data = date_range, aes(x = date, y = predicted_cases), color = "blue") + # Use geom_line with predictions
  scale_x_date()
```
...
print(model_2)
```
Using 5 fold CV we improved the R2 value of the model from 0.11 to 0.14 however this value is still too low to be considered a good model. An effective model usually has an R2 value of higher than 0.7.
